...
A single-file, offline suite of networking tools and labs. No build step, no server, no external requests — open [`index.html`](./index.html) in a browser and everything runs client-side.

## Log Sanitizer

The tool this kit is built around. Paste a log, config dump, or command output and it finds and redacts sensitive values in real time — internal IPs, hostnames, usernames, API keys, tokens, passwords, and anything matching a custom pattern — before you paste that log into a support ticket, a vendor case, a chat message, or an AI tool.
...
- **Extra-sandboxed.** The Log Sanitizer's iframe runs with a restrictive sandbox (`allow-scripts allow-modals allow-downloads allow-popups`, deliberately missing `allow-same-origin`) so the frame holding your secrets is opaque even to the kit's own parent page.
...
Just open `index.html`. Each tool is embedded inline and loaded into an iframe on demand — there's nothing to install and nothing to configure.
